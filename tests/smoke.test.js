...
describe('website smoke test', () => {
  ...
  // views (inventory-app#1731): the tiles, the dependent options and the
  ...
  it('renders the collection results on the composed results view', async () => {
    ...
    // Nine a page, more than one page (142 items in the package).
  ...
  it('renders a keyword search on the composed results view', async () => {
    ...
    // This package's own names carry Markdown emphasis (e.g. `*Alfarje*
    // ceiling.`) which the rendered "you searched for" text displays without
    // the asterisks — stripped here so the assertion matches what renders.
    const term = items[0].internal_name.split(' ')[0].replace(/[*_]/g, '')
  ...
  it('renders the about page on the composed text page view', async () => {
    ...
    expect(host.textContent).toContain('Architectural Elements')
  ...
  it('renders the credits page on the composed text page view', async () => {
    ...
    expect(host.textContent).toContain('MWNF TEAM')
    expect(host.querySelector('.mwnf-prose')).not.toBeNull()
    app.unmount()
  }, 20000)
  ...
  it('shows the explore-partner notice only for the project dataset.config.js lists', async () => {
    const epm = await mountSite('#/item/78426513-4220-51b8-99aa-4f7ba05f6c3a')
  ...
  // that project's URL is non-null. architectural-elements-data 1.0.1
  // carries both URLs for Discover Islamic Art, so this asserts the positive
  // case against the package's own values rather than a hard-coded URL.
  ...
  // inventory-app#1727 phase 4: `useCollection.js`'s tile meta line reads
  // the borrowed item's project name off the manifest too.
  it('shows the source project on a collection-results tile, from the manifest', async () => {
    ...
    const term = item.internal_name.replace(/[*_]/g, '')
    const { app, host } = await mountSite(`#/search?q=${encodeURIComponent(term)}`)
    await vi.waitFor(() => expect(host.querySelector('.mwnf-grid__tile')).not.toBeNull(), { timeout: 20000 })
  ...
  // slot are the layout's own: `GlossaryTool` and `DynastyList` from
  // `@museumwnf/viewer-layout/content`, each a native `<details>` toggle
  // with its own heading text.
  it('renders the layout glossary tool and dynasty popouts on the item sheet', async () => {
    // An *Alfarje* ceiling with the Mudéjar period dynasty attached — a
    // record whose dynasty translation actually carries a history, so it
    // survives ItemSheet.vue's own "has something to show" filter, and whose
    // glossary_ids are non-empty.
    const { app, host } = await mountSite('#/item/f88193ec-c618-5df9-8728-cdb15d419ec4')
    ...
    expect(host.textContent).toContain('Mudéjar period')
    app.unmount()
  }, 60000)
  ...
  it('renders the timeline results on the composed timeline view', async () => {
    ...
    await vi.waitFor(() => expect(host.textContent).toContain('Druze leader'), { timeout: 20000 })
    // The join to member items finds Lebanon's two dated objects even
    // with no period chosen — "all countries too" behaviour.
  ...
  it('renders the timeline gallery on the composed results view', async () => {
    ...
    expect(host.textContent).toContain('Fountain')
    app.unmount()
  }, 60000)

  // The composed entrance's own country control writes the inventory id
  // (`lbn`), not the legacy two-letter code (`lb`) the gallery's own deep
  ...
  it('renders the partners list on the composed partner-list view', async () => {
    ...
    expect(host.textContent).toContain('Spain')
    expect(host.textContent).toContain('National Archaeological Museum')
    // The object count is this website's own copy over the shared entry, not
    // a generic count.
    expect(host.textContent).toContain('11 object(s) in this site')
    app.unmount()
  }, 60000)

  it('renders a partner profile on the composed record view', async () => {
    const { app, host } = await mountSite('#/partner/7712e497-33e4-5529-91c3-47abff00f87c')
    ...
    await vi.waitFor(() => expect(host.textContent).toContain('National Archaeological Museum'), { timeout: 20000 })
    expect(host.textContent).toContain('Madrid')
    // The map is the layout's `PartnerMap`, not a local component.
  ...
  it("renders a partner's objects on the composed grid results view", async () => {
    const { app, host } = await mountSite('#/partner/7712e497-33e4-5529-91c3-47abff00f87c/objects')
  ...
  // these 14 entries (every one but home/item/timeline). Every existing deep
  // link and `legacyRoutes` resolver targets these name/path pairs, so they
  // must not move — this pins the exact set `config.extraViews` declares
  // (the router's own generated `legacy-*`/`not-found` entries are not this
  // website's own routes, so they are not part of the pin).
  ...
  // Every route needs a section for the menu to know where it is (the
  // shell's `useSection()`), which the config-driven `SiteShell` also relies
  // on for the active menu entry and the banner-title fallback.
  it('gives every route a section', () => {
    expect(checkSectionMeta(config)).toEqual([])
  })

  // The one behaviour a config-driven shell could silently lose: the
  // layout's `SiteShell` marks a `config.navigation.links` entry active by
  // comparing its own `section` to `useSection()`, exactly as a local shell
  // would.
  it('marks the current section active in the menu, and renders the header, footer and search', async () => {
    ...
    // `config.navigation`'s, not a local computed list.
  ...
  // What this website contributes to a legacy address is the mapping: a
  // dbUid path to an item, a country and legacy id to a partner, the page
  // number out of the path. That the router turns such an entry into a
  // redirect is viewer-core's own test.
  ...
  // The chrome is two layers now, and either one failing is silent: a
  // missing entry renders as its own name rather than as an error. This
  // asserts the rendered page, so a bundle that installs but never reaches
  ...
  it('renders the shared texts and its own over them', async () => {
    ...
    // own — a raw shared key otherwise passes unseen: the record/sheet/
    // timeline/partner pages read the neutral namespaces directly, on top of
    // `gallery`, this site's own product section.
    expect(checkTextsRendered(host, {
      namespaces: ['architecturalElements', 'core', 'layout', 'catalogue', 'record', 'sheet', 'timeline', 'partner', 'gallery'],
    })).toEqual([])
