...
<template>
  <SiteShell
    v-bind="$attrs"
    header-home="#/"
    :footer-text="t('architecturalElements.identity.copyright')"
  >
    <template #brand>
      <span class="site-logo">
        <span class="site-logo-org">{{ t('architecturalElements.identity.organisation') }}</span>
        <span class="site-logo-title">{{ t('architecturalElements.identity.title') }}</span>
      </span>
    </template>
    <slot />
  </SiteShell>
