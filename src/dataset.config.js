...
export default {
  ...
  // addresses every DXA config repeats, Architectural Elements adding none of
  // its own.
  links: { ...mwnfLinks },

  // The source-database chip's colour, per project (epic #1727 phase 4):
  // one entry for every project id `npm pack @museumwnf/architectural-elements-data`
  // carries in `manifest.projects` (checked 2026-09-22, architectural-elements-data
  ...
  // own — every member is borrowed, same as amulets and coins-medals.
  projectColors: {
    '09bed55d-efe6-50e0-8c24-1647bc9822eb': 'mwnf-chip--EXH', // The Table Is Set — a borrowed thematic exhibition, not a gallery project
    '0f031e22-6dc6-5ce6-b94b-9bb88345140c': 'mwnf-chip--AWE', // Sharing History - Arab-Ottoman-European relations in the 19th century.
    '21cccf03-49f5-55d3-96f5-eddeee7a989d': 'mwnf-chip--EXH', // Water in Islam — a borrowed thematic exhibition, not a gallery project
  ...
  // own editorial choice, not a literal legacy key check. Carpets, amulets
  // and coins-medals share this list by construction — all three borrow from
  // the same Explore Islamic Art Collections project; Architectural Elements,
  // itself a borrower of that project, follows the same choice.
  ...
  site: { origin: 'https://museumwithnofrontiers.github.io/architectural-elements' },
  ...
  extraViews: [
    ...
    // results/gallery) is byte-identical to carpets', amulets' and
    // coins-medals' own on `origin/main` (epic inventory-app#1731) — the
    // package's shared gallery pages, spread in with this site's one genuine
    // per-site string (`Credits.vue`'s `body` entry name).
    ...standardRoutes('gallery', { creditsBody: 'architecturalElements.credits.body' }),
  ],
