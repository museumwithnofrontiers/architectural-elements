...
The **Museum With No Frontiers — Architectural Elements** gallery (legacy env
key `arh`): a gallery built entirely from borrowed material — it holds no
native project of its own, only items, partners, dynasties and timeline
events on loan from other MWNF databases (Discover Islamic Art, Explore
Islamic Art Collections, Discover Glass Art, Sharing History, The Table Is
Set, Water in Islam, The Hijaz Railway) — the collection, its dependent
filters, a full-text database search, partner profiles and their holdings,
country timelines and the item sheets themselves, all built from the
published dataset.
...
| Package | Role |
| --- | --- |
| `@museumwnf/architectural-elements-data` | the dataset (JSON + `manifest.json`) |
...
| `src/views/` | the three pages this gallery still owns (`Home.vue`, `ItemSheet.vue`, `Timeline.vue`) — every other page is the platform's own shared gallery page |
| `src/composables/gallery.js` | this gallery's one instance of the shared DXA gallery data layer, plus the item-sheet spec extensions that are genuinely this gallery's own (source-database chip, Explore-partner notice, holding-museum link, related-content block) |
...
overloaded there with the Architectural Elements text (it explains, among
other things, that this gallery holds no native collection of its own), while
`architecturalElements.credits.body` is a name of this gallery's own, because
there is no generic credits page to inherit.
...
Node runs on the host for this repository (no Docker in this lane).
...
Follow the same pencil-button flow as above for small changes, or run
`npm run dev` for real design work. A change to a layout component itself is a
request for the `viewer-layout` package — open an issue there.

## Deployment

Every push to `main` builds and publishes the site to
<https://museumwithnofrontiers.github.io/architectural-elements/> through the reusable workflows in
